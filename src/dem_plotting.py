...
def compare_with_data(dem_wave, dem_flux, dem_err,
    ...
    fdata = np.zeros(len(lines))
    fdem = np.zeros(len(lines))
    ...
    for i in range(len(lines)):
        ...
        fdata[i] = np.log10(flux_data)
        fdem[i] = np.log10(flux_dem)

    return fdata, fdem


def compare_with_table(dem_wave, dem_flux, dem_err,
                       table, wavekey, fluxkey,
                       distance, ax, c='k', label='',
                       binsize=None):
    if binsize is None:
        binsize = np.full(len(table), 1)

    fdata = np.zeros(len(table))
    fdem = np.zeros(len(table))

    for i in range(len(table)):
        dem_q = ( (dem_wave > table[wavekey][i]-binsize[i]) &
                  (dem_wave < table[wavekey][i]+binsize[i]) )

        flux_dem  = np.trapz(dem_flux[dem_q], dem_wave[dem_q]) * 4*np.pi*distance**2
        err_dem   = np.trapz(dem_err[dem_q], dem_wave[dem_q])  * 4*np.pi*distance**2

        ax.errorbar(np.log10(table[fluxkey][i]*4*np.pi*distance**2),
                    np.log10(flux_dem),
                    #xerr=np.abs(np.log10(flux_data) - np.log10(err_data)),
                    #yerr=np.abs(np.log10(flux_dem) - np.log10(err_dem)),
                    marker='o', c=c, ms=8, linestyle='')
        fdem[i] = np.log10(flux_dem)
        fdata[i] = np.log10(table[fluxkey][i]*4*np.pi*distance**2)
